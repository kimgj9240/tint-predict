...
# 1. 사전에 로딩된 LSTM 모델
model = load_model('./new_model/24-0.8592.h5',compile=False)

# 2. 유저 입력 -> LSTM 입력 (3일치 X 8피처) 변환 함수
...
def predict():
    try:
        ...
        # 클라이언트가 보낸 JSON 데이터를 딕셔너리 형태로 파싱 예) {"orders":[{"date":"2025-04-09","qty":100}, ...]}
    ...
    # 오류 발생시 HTTP 500 응답과 함께 에러 메시지 반환

# 4. 서버 실행
if __name__ == "__main__":
    app.run(host='0.0.0.0', port=5000)

# 기본적으로 localhost:5000에서 실행됨, port=5000 생략 가능
